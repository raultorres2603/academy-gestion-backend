    ...
    connection.query(
      ...
      (err, resu) => {
        if (err) {
          ...
          if (resu[0].cont == 0) {
            connection.query(
              `INSERT INTO users(username, password, type) VALUES('${req.body.username.toUpperCase()}','${req.body.password.toUpperCase()}', 1)`,
              (err, resu) => {
                if (err) {
...
router.post("/userInfo", function (req, res, next) {
  connection.query(
    `SELECT firstName, idUser, secondName, type, nif, tel, country, city FROM users WHERE idUser = ${req.body.idUser}`,
    (err, resu) => {
      if (err) {
        res.send(JSON.stringify({ error: "1" }));
        ...
        res.send(
          JSON.stringify({
            firstName: resu[0].firstName,
            secondName: resu[0].secondName,
            type: resu[0].type,
            nif: resu[0].nif,
            tel: resu[0].tel,
...
router.post("/updateUser", function (req, res, next) {
  connection.query(
    `UPDATE users SET firstName = '${req.body.firstName}', secondName = '${req.body.secondName}', nif = '${req.body.nif}', tel = '${req.body.tel}', country = '${req.body.country}', city = '${req.body.city}' WHERE idUser = ${req.body.idUser}`,
    (err, resu) => {
      if (err) {
        res.send(JSON.stringify({ error: "1" }));
        console.log(JSON.stringify({ error: "1" }));
